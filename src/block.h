	...
	Type *New(int num = 1)
	{
		...
		if (!last || last->current + num > last->last)
		{
			...
			else
			{
				...
				if (!next) { if (error_function) (*error_function)("Not enough memory!"); Rcpp::stop("Not enough memory!"); }
...
public:
	...
	Type *New()
	{
		...
		if (!first_free)
		{
			...
			if (!first) { if (error_function) (*error_function)("Not enough memory!"); Rcpp::stop("Not enough memory!"); }
