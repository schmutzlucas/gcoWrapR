...
void GCoptimization::setDataCost(DataCostFnExtra fn, Rcpp::List extraData) {
  specializeDataCostFunctor(DataCostFnFromFunctionExtra(fn, extraData));
  m_labelingInfoDirty = true;
}


void GCoptimization::setDataCostRcpp(SEXP fn_, Rcpp::List extraData) {
  DataCostFnExtra fn = *Rcpp::XPtr<DataCostFnExtra>(fn_);
  // EnergyTermType (*DataCostFnExtra)(SiteID s, LabelID l,void *)
  specializeDataCostFunctor(DataCostFnFromFunctionExtra(fn, extraData));
}
...
void GCoptimization::setSmoothCost(SmoothCostFnExtra fn, Rcpp::List extraData) {
  specializeSmoothCostFunctor(SmoothCostFnFromFunctionExtra(fn, extraData));
}

//-------------------------------------------------------------------

void GCoptimization::setSmoothCostRcpp(SEXP fn_, Rcpp::List extraData) {
  SmoothCostFnExtra fn = *Rcpp::XPtr<SmoothCostFnExtra>(fn_);
  specializeSmoothCostFunctor(SmoothCostFnFromFunctionExtra(fn, extraData));
}
...
void GCoptimization::permuteLabelTable()
{
  ...
  std::mt19937 rng(dev());
  std::uniform_int_distribution<std::mt19937::result_type> uint_dist;
	if ( !m_random_label_order )
		return;
	for ( LabelID i = 0; i < m_num_labels; i++ )
	{
		LabelID j = i + (uint_dist(rng) % (m_num_labels-i));
...
void GCoptimization::printStatus1(const char* extraMsg)
{
	...
	if ( extraMsg )
		Rprintf("gco>> %s\n",extraMsg);
	Rprintf("gco>> initial energy: \tE=%f (E=%f+%f+%f)\n",(long long)compute_energy(),
...
void GCoptimization::printStatus1(int cycle, bool isSwap, gcoclock_t ticks0)
{
	...
	Rprintf("gco>> after cycle %2i: \tE=%f (E=%f+%f+%f);",cycle,(long long)compute_energy(),
		(long long)giveDataEnergy(),(long long)giveSmoothEnergy(),(long long)giveLabelEnergy());
	if ( m_stepsThisCycleTotal > 0 )
		Rprintf(isSwap ? " \t%i swaps(s);" : " \t%i expansions(s);",m_stepsThisCycleTotal);
	if ( m_verbosity == 1 )
	{
...
void GCoptimization::printStatus2(int alpha, int beta, int numVars, gcoclock_t ticks0)
{
	...
	if ( beta >= 0 )
		Rprintf("gco>>   after swap(%i,%i):",alpha+INDEX0,beta+INDEX0);
	else
		Rprintf("gco>>   after expansion(%i):",alpha+INDEX0);
	Rprintf(" \tE=%f (E=%f%f+%f);\t %lld vars;",
		(long long)compute_energy(),(long long)giveDataEnergy(),
		(long long)giveSmoothEnergy(),(long long)giveLabelEnergy(),(long long)numVars);
	if ( m_stepsThisCycleTotal > 0 )
		Rprintf(" \t(%i of %i);",m_stepsThisCycle+1,m_stepsThisCycleTotal);

	Rprintf(microsec > 100 ? "\t %.2f ms\n" : "\t %.3f ms\n",(double)microsec/1000.0);
	flushnow();
}
